...
public class UserPublishFragment extends Fragment {
				...
				try {
					...
					if (js.getString("code").equals(hpCantant.SUCCESS_CODE)) {
						...
						try {
							...
							Toast.makeText(getActivity(), "暂无发布",
	...
	private void getData() {
		...
		map.put("lat", ApplicationController.getInstance().getLat());// 经度
		map.put("lng", ApplicationController.getInstance().getLng());// 维度
		map.put("countid", ApplicationController.getInstance().getUser()
				.getInt("countid", 0)
				+ "");// 用户id
		map.put("way", "2");// 获取方式1.为我举办的 2为我参与的
